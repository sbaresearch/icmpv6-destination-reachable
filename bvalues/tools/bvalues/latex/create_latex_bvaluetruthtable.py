...
def read_json_data(BASE_PATH,protocol, date):
    file_path = os.path.join(BASE_PATH, f'bvalue_vantage1/{date.strftime("%Y_%m_%d")}/{protocol}/bvalue_changes_counts.json')
...
def generate_table_types_responses(BASE_PATH):
    # Initialize results dictionary for each protocol, response number, and message type count
    results = {protocol: {str(resp_num): {str(i): [] for i in range(0,6)} for resp_num in range(0, 6)} for protocol in PROTOCOLS}
    print(results)
    # Collect data
    for date in date_range(START_DATE, END_DATE):
        for protocol in PROTOCOLS:
            data = read_json_data(BASE_PATH,protocol, date)
    ...
    # Calculate means
    total_per_protocol = {protocol: 0 for protocol in PROTOCOLS}
    ...
    for protocol in PROTOCOLS:
        for resp_num in range(0, 6):
            ...
            for msg_type in range(1,6):
                ...
                mean_results[protocol][resp_num_str][msg_type_str] = mean_value
                total_per_protocol[protocol] += mean_value
    ...
    print("\\end{tabular}")
    print("\\end{tiny}")
    print("\\caption{Mean number responses in relation to number of message types for BValue Steps.}")
    print("\\label{table:mean_subnet_responses_matrix}")
    print("\\end{table}")

    print("-----------------------------------------")

    # Output LaTeX Chart table
    print("\\begin{table}")
    print("\\begin{tiny}")
    print("\\begin{tabular}{cc|c||c|c|c|c|c|c|}")
    print("& & &  \\multicolumn{5}{c|}{No. of responses} \\\\")
    ...
    for msg_type in range(1, 6):  # Message types
        for protocol in PROTOCOLS:
            ...
            row_label = f"& {msg_type} & {protocol.upper()}"
            for resp_num in range(1, 6):                
                mean_value = mean_results[protocol][str(resp_num)][str(msg_type)]
                if total_per_protocol[protocol] != 0:
                    percentage = mean_value / total_per_protocol[protocol]
                else:
                    percentage = 0
                counts += f' & \\Chart{{{percentage:.2f}}}'
            print(f"{row_label} {counts} \\\\")
            if protocol == PROTOCOLS[-1]:
                print("\\cline{2-8}")
...
def main():
    ...
    generate_table_types_responses(BASE_PATH)
